...
class DQNAgent:
    ...
    def init_model(self):
        ...
        x = Convolution2D(16, 4, 4, border_mode='same', sctivation='relu',
                          subsample=(2, 2))(state_input)
        x = Convolution2D(32, 2, 2, border_mode='same', sctivation='relu',
                          subsample=(1, 1))(x)
        x = Convolution2D(32, 2, 2, border_mode='same', sctivation='relu',
                          subsample=(1, 1))(x)
        x = Flatten()(x)
        ...
        y_pred = Dense(self.n_action, activation='linear', name='main_output')(x)
    ...
    def init_simple_model(self):
        ...
        y_true = Input(shape=(3,), name='y_true')
        loss_out = Lambda(loss_func, output_shape=(1, ), name='loss')([y_true, y_pred, action_step])
        self.model = Model(input=[state_input, action_input, y_true], output=[loss_out, y_pred])
        ...
        self.model.compile(loss=losses,
                           optimizer=optimizer(lr=self.learning_rate),
                           metrics=['accuracy'])
        self.targe_model = copy.copy(self.model)
        self.summary_op = tf.summary.merge_all()
        self.summary_writer = tf.summary.FileWriter('./log', graph=self.session.graph)
    ...
    def Q_values(self, states, isTarget=False):
        model = self.targe_model if isTarget else self.model
        res = model.predict({'state': np.array([state]),
                             'action': np.array([0]),
                             'y_true': np.array([[0] * self.n_action])
                             })
        return res[1][0]

    def update_target_model(self):
        self.targe_model = clone_model(self.model)
    ...
    def experience_replay(self, step, score=None):
        ...
        # simple armdom minibatch
        minibatch_size = min(len(self.D), self.minibatch_size)
        minibatch_indexes = self.random.randint(0, len(self.D), minibatch_size)
