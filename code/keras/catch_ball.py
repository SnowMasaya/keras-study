...
class CatchBall:
    ...
    def update(self, action):
        ...
        if self.ball_past_time == self.ball_post_interval:
            ...
            if not self.simple:
                while len(self.balls) > 0 and \
                    abs(new_pos -self.balls[-1].col) > \
                                                self.ball_post_interval + self.player_length - 1:
                    new_pos = np.random.randint(self.screen_n_cols)
        ...
        if self.balls[0].row == self.screen_n_rows - 1:
            if self.player_col <= \
                    self.balls[0].cols < self.player_col + self.player_length:
                # catch
                self.reward = 1
            else:
                # drop
